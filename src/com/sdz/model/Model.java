...
public class Model implements Observable {
		...
		if (this.mot.verifyWord(c) != -1) {
			...
			if (this.mot.isFinished()) {
				...
				JOptionPane.showMessageDialog(null,
						"Vous avez trouvé le mot " + this.mot.getWord() + " en " + this.mot.getNombreCoup() + " coup"
								+ ((this.mot.getNombreCoup() > 1) ? "s" : "") + ", avec " + this.mot.getNombreErreur()
			...
			if (this.mot.getNombreErreur() == 7) {
				...
				JOptionPane.showMessageDialog(null, "Le mot était :\n\t" + this.mot.getWord(), "Vous avez perdu",
						JOptionPane.NO_OPTION);

				if (this.scoreSerializer.isAccpeted(this.score)) {
					String nom = "";
					nom = (String) JOptionPane.showInputDialog(null, "Entrez votre pseudonyme :", "Félicitations",
	...
	public void notifyObserver() {
		...
		for (Observer obs : this.listObserver)
			obs.update(this.mot.getSecretWord(), this.score.getPoint(),
					"images/pendu" + this.mot.getNombreErreur() + ".jpg", this.nombreMot);
	}
